...
public class getPackDetails extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) {
		{
			...
			try {
				...
				Log.getRequestResponseReccLog(strFlag, strSessionId + "," + strMsisdn
						+ "," + circleId + "," + strReqType + "," + strConnType, "Request");

				url = "http://" + strIP + ":" + strPort + "/services/TisService";
				System.out.println("PackDetails-->Circle ::" + strFlag + ", Url ::"
						+ url);
								...
								for (int i = 0; i < resMessage.size(); i++) {
									try {
										...
										if (!isCheckBlackListedPacks(strPackName, strBL_Packs)) {
											...
											System.out.println("Packname==>"+strPackName+",Circle==>"+circleId+",PackDescription==>"+packDesc);
											j = j + 1;
											if (packDesc == "") {
												...
												if (channel.containsKey(first.toString())) {
													...
													String data = "";
													if (use.containsKey(second.toString())) // I(mobile internet),B(Mobile broadband),P(Shared parent),C(shared child)
													{
														...
														if (use.containsKey(packType)) //2g,3g,wifi
														{
															...
															if (validity != null
																	|| (!validity.equalsIgnoreCase("")))//validity check
															{
																if (rental != null
																		|| (!rental.equalsIgnoreCase("")))//rental check
																{
																	...
																	else if (strPackName.contains("D"))//data check
																	{
																		data = packDesc(strPackName);
																		strMessage = strMessage
																				+ (j + "." + use.get(second.toString())
									...
									try {
										...
										if (!isCheckBlackListedPacks(strPackName, strBL_Packs)) {
											...
											if (channel.containsKey(first.toString())) {
												...
												if (use.containsKey(second.toString())) // I(mobile internet),B(Mobile broadband),P(Shared parent),C(shared child)
												{
													...
													if (use.containsKey(packType)) //2g,3g,wifi
													{
														...
														if (tempValidity != null
																|| (!tempValidity.equalsIgnoreCase("")))//validity check
														{
															if (rental != null
																	|| (!rental.equalsIgnoreCase("")))//rental check
															{
																...
																if (strPackName.contains("T"))//data check
																{
																	//f	strPackName=strPackName.substring(strPackName.lastIndexOf("T"),strPackName.length());
																	//	strPackName=strPackName.replace("T","D");
																	if (strPackName.contains("N")) {
																		if (strPackName.contains("UL")) {
																			strMessage = strMessage
																					+ (j
																							+ ".Hi, You have consumed "
																							+ getBalanceOfDuration(balance / 1024)
																							+ "MB from your "
																							+ use.get(packType)
																							+ " Unlimited Pack.It will expire on "
																							+ validity + ".~ ");
																		} else {
																			accBalance = getBalanceOfDuration(balance);
																			strMessage = strMessage
																					+ (j
																							+ ".Hi, Your data Usage is "
																							+ usuage(strPackName, accBalance)
																							+ " & Balance is "
																							+ getBalanceOfDuration(balance / 1024)
																							+ "MB" + "." + use.get(packType)
																							+ " Night Pack will expire on "
																							+ validity + ".~ ");
																		}
																		...
																		strMessage = strMessage
																				+ (j
																						+ ".Hi, You have "
																						+ remaining_bal
																						+ " time left for free data usage in your Current "
																						+ use.get(packType) + " Hourly Pack .~ ");
																	}

																} else if (strPackName.contains("C"))//data check
																{
																	if (strPackName.contains("N")) {
																		if (strPackName.contains("UL")) {
																			strMessage = strMessage
																...
																} else if (strPackName.contains("DB"))//data check
																{
																	if (strPackName.contains("N")) {
																		if (strPackName.contains("UL")) {
																			strMessage = strMessage
														...
														else//volume add on packs
														{
															if (strPackName.contains("N")) {
																...
																strMessage = strMessage
																		+ (j + ".Hi, You have consumed "
																				+ getBalanceOfDuration(balance / 1024)
																				+ "MB from your " + use.get(packType)
																				+ " Unlimited Pack.It will expire on "
																				+ validity + ".~ ");
															}
																...
																accBalance = getBalanceOfDuration(balance);
																strMessage = strMessage
																		+ (j + ".Hi, Your data Usage is "
																				+ usuage(strPackName, accBalance)
																				+ " & Balance is "
																				+ getBalanceOfDuration(balance / 1024)
																				+ "MB" + "." + use.get(packType)
																				+ " Pack will expire on " + validity + ".~ ");
															}
														}
													} else//rat products
													{
												...
												{
													strMessage = strMessage
															+ (j + ".Hi, Your data balance is "
																	+ getBalanceOfDuration(balance / 1024) + "MB"
																	+ " for Booster Pack will expire on "
																	+ validity + ".~ ");
												}
											}
											...
											if (!isCheckBlackListedPacks(strPackName, strBL_Packs)) {
												...
												if (secondDigit == 'I' && thirdDigit == 'W'
													...
													if ((second_last == 'U' && last_digit == 'L')
														...
														//  strMessage = j + "." + "Hello, You have " + balance + " MB free usage balance, in your WiFi pack " + strPackName + ", Valid till " + validity + ".~ ";
														strMessage = j + "." + conn_type + " "
	...
	public static String packDesc(String strPackName) {
		...
		} else if (strPackName.contains("P"))//rat based product
		{
	...
	public static String usuage(String strPackName, double balance) {
		//System.out.println("PackName=" + strPackName + ",Balance= " + balance);
		String data = "";
		double packvalue = 0l;
		double temp;
		String usuage;
		...
		else if (strPackName.contains("P") && strPackName.contains("G"))//rat based product
		{
			String data1 = strPackName.substring(strPackName.lastIndexOf("D") + 1,
					strPackName.lastIndexOf("P"));
			String data2 = strPackName.substring(strPackName.lastIndexOf("P") + 1,
		...
		if (strPackName.endsWith("HM"))//MB CHECK
		{
			data = strPackName.substring(strPackName.lastIndexOf("D") + 1,
					strPackName.lastIndexOf("H"));
			packvalue = Integer.parseInt(data) * 100 * 1024;
		} else if (strPackName.endsWith("KM"))//mb check
		{
			data = strPackName.substring(strPackName.lastIndexOf("D") + 1,
					strPackName.lastIndexOf("K"));
		...
		usuage = temp + "" + "MB";
		System.out.println("PackValue==>"+packvalue + ",PackName==>" + strPackName + ",Usuage==>" + usuage
				+ ",Balance==>" + balance);
